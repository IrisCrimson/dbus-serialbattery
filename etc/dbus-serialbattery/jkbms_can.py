...
class Jkbms_can(Battery):
    ...
    CAN_FRAMES = {BATT_STAT: 0x02f4, CELL_VOLT: 0x04f4, CELL_TEMP: 0x05f4, ALM_INFO: 0x07f4}
    ...
    def to_protection_bits(self, byte_data):
        ...
        self.protection.voltage_cell_low = 2 if int(tmp[pos-4:pos-2],2) > 0 else 0
        ...
        self.protection.temp_high_charge = 2 if int(tmp[pos-16:pos-14],2) > 0 else 0
        self.protection.temp_high_discharge = 2 if int(tmp[pos-16:pos-14],2) > 0 else 0
        self.protection.temp_low_charge = 2 if int(tmp[pos-18:pos-16],2) > 0 else 0
        self.protection.temp_low_discharge = 2 if int(tmp[pos-18:pos-16],2) > 0 else 0
        self.protection.temp_high_charge = 2 if int(tmp[pos-20:pos-18],2) > 0 else 0
        self.protection.temp_high_discharge = 2 if int(tmp[pos-20:pos-18],2) > 0 else 0
    ...
    def read_serial_data_jkbms_can(self):
        if self._can_bus is False:
            ...
            try :
                self._can_bus = can.interface.Bus(bustype=CAN_BUS_TYPE, channel=self.port, bitrate=self.baud_rate)
        ...
        while messages_to_read > 0:
            ...
            if msg is not None:
                ...
                if msg.arbitration_id == self.CAN_FRAMES[self.BATT_STAT]:
                    ...
                    alarms =  unpack_from('<L', bytes([msg.data[0], msg.data[1], msg.data[2], msg.data[3]]))[0]
