...
class Jkbms_Can(Battery):
    ...
    def __init__(self, port, baud, address):
        ...
        if address is None:
            self.can_frames = {
                self.BATT_STAT: int("0x2F4",16) + self.CAN_FRAMES_OFFSETS[self.BATT_STAT], # 0x02F4
                self.CELL_VOLT: int("0x2F4",16) + self.CAN_FRAMES_OFFSETS[self.CELL_VOLT], # 0x04F4
                self.CELL_TEMP: int("0x2F4",16) + self.CAN_FRAMES_OFFSETS[self.CELL_TEMP], # 0x05F4 
                self.ALM_INFO:  int("0x2F4",16) + self.CAN_FRAMES_OFFSETS[self.ALM_INFO],  # 0x07F4
            }
        else:
            self.can_frames = {
                self.BATT_STAT: self.address + self.CAN_FRAMES_OFFSETS[self.BATT_STAT], # 0x02F4
                self.CELL_VOLT: self.address + self.CAN_FRAMES_OFFSETS[self.CELL_VOLT], # 0x04F4
                self.CELL_TEMP: self.address + self.CAN_FRAMES_OFFSETS[self.CELL_TEMP], # 0x05F4 
                self.ALM_INFO:  self.address + self.CAN_FRAMES_OFFSETS[self.ALM_INFO],  # 0x07F4
            }
    ...
    def to_protection_bits(self, byte_data):
        ...
        pos = len(tmp)
        logger.debug(tmp)
        self.protection.cell_imbalance |= 2 if int(tmp[pos - 2 : pos], 2) > 0 else 0
        self.protection.voltage_cell_low |= (
            2 if int(tmp[pos - 4 : pos - 2], 2) > 0 else 0
        )
        self.protection.voltage_high    |= 2 if int(tmp[pos - 6 : pos - 4], 2) > 0 else 0
